...
namespace ipchanger {
	...
	bool Changer::SearchAndReplace(std::string& in, const std::string_view& pattern, const std::string_view& replace)
	{
	...
	void Changer::SearchAndReplace(std::string& in, unsigned int pattern, unsigned int replace)
	{
		for (char& i : in) {
			if (*reinterpret_cast<unsigned int*>(&i) == pattern)
				*reinterpret_cast<unsigned int*>(&i) = replace;
		}
	}

	Changer::Changer(const std::string& ip, unsigned int port, const fs::path& in)
